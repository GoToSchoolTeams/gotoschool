...
function GetStandFileName(who, pause, face, bust, big)
{
	if(big == 'true')
	{
		who = who + "_big";
	...
	var filename = who + "_" + "p" + pause + "_" + face;

	if(bust == 'true')
	{
		filename = "bust_" + filename;
	}
...
function SaveStandInfo(who, pause, face)
{
	var dispname = tf.CharaNameKanziMap[who];
	delete(f.lastShowStandImageinfo[dispname]);
	f.lastShowStandImageinfo[dispname] = new global.StandImageInfo(who, pause, face);
}
...
function GetBustUpImageNameFromStandInfo(who)
{
	...
	if(info != void) {
		return global.GetStandFileName(info.name, info.pause, info.face, true, false);
	}
