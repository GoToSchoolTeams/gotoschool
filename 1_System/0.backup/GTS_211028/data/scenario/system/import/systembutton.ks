...
class SystemButtonPlugin extends KAGPlugin
{
	...
	function createButtons(parent, array)
	{
		...
		obj.loadImages('SYS_button_message.png'); // SYS用画像を読み込む

		//log
		array.add(obj = new SystemButtonLayer(kag, parent, onLogMode));
		obj.loadImages('LOG_button_message.png'); // メッセージ履歴用画像を読み込む
		
		//save
		array.add(obj = new SystemButtonLayer(kag, parent, onSaveButtonClick));
		obj.loadImages('SAVE_button_message.png'); // save用画像を読み込む
		
		//load
		array.add(obj = new SystemButtonLayer(kag, parent, onLoadButtonClick));
		obj.loadImages('LOAD_button_message.png'); // load用画像を読み込む
		
		//auto
		array.add(obj = new SystemButtonLayer(kag, parent, onAutoMode));
		obj.loadImages('AUTO_button_message.png'); // オートモード用画像を読み込む
		
		//skip
		array.add(obj = new SystemButtonLayer(kag, parent, onSkipToNextStop));
		obj.loadImages('SKIP_button_message.png'); // スキップ用画像を読み込む

		//Exit
		array.add(obj = new SystemButtonLayer(kag, parent, onExitClick));
		obj.loadImages('EXIT_button_message.png'); // Exit用の画像を読み込む
	...
	function realign()
	{
		...
		for(var i = 0; i < count; i++)
		{
			...
			obj = backButtons[i];
			obj.setPos(xpos, y);
			obj.absolute = 2000000-2; // 重ね合わせ順序はメッセージ履歴よりも奥

			obj = foreButtons[i];
			obj.setPos(xpos, y);
			obj.absolute = 2000000-2;

			btn_x += obj.width+5;//文字列間の長さを決める
		}
	}
	...
	function onSaveButtonClick()
	{
		...
		kag.callExtraConductor("SysbuttonJump.ks","*save.ks"); // save画面用サブルーチンを呼び出す
	}
	...
	function onLoadButtonClick()
	{
		...
		kag.callExtraConductor("SysbuttonJump.ks","*Load.ks");//load画面用サブルーチンを呼び出す
	}
	...
	{
  		kag.callExtraConductor("SysbuttonJump.ks","*Config.ks");  // コンフィグ画面用サブルーチンを呼び出す
	}
	...
	{	
		kag.callExtraConductor("Exit.ks");
	}
...
;
;;systembutton描画\n
;;forevisible=表画面表示,true;false
;;backvisible=裏画面表示,true;false
@macro name="sysbtopt"
@eval exp="systembutton_object.setOptions(mp)"
; mp がマクロに渡された属性を示す辞書配列オブジェクト
@image cond="0" forevisible=%forevisible backvisible=%backvisible left=%left top=%top
@endmacro
